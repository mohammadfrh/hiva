...
class AutoTradeActivity : AppCompatActivity() {
    ...
    private fun bind(state: AutoTradeUiState) {
        ...
        textSession.text = buildString {
            ...
            appendLine("last close: ${state.lastCloseStatus.ifBlank { "-" }}")
            appendLine("last close at: ${formatEpochSec(state.lastCloseAtEpochSec)}")
            appendLine("last close refPx: ${formatPrice(state.lastCloseEngineRefPrice)}")
            appendLine("last risk sync: ${state.lastRiskSyncStatus.ifBlank { "-" }}")
            appendLine("last risk sync at: ${formatEpochSec(state.lastRiskSyncAtEpochSec)}")
